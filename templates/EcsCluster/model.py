...
class ScalingGroupModel(BaseModel):
    ...
    in_default_cps = Field(
        True,
        description="When true, this scaling group will participate in the default capacity provider strategy for the cluster. This option has no effect if `auto_scaling_enabled` is false.",
        notes=[
    ...
    weight = Field(
        1,
        description="Sets the weight of the scaling group within the default capacity provider strategy. This option has no effect if `auto_scaling_enabled` is false.",
        notes=[
...
class UserDataModel(BaseModel):
    ...
    ingress_cidrs: List[str] = Field(
        [],
        description="List of CIDRs which will be allowed to communicate with the container instances. This must include at least the load-balancer subnets which will be associated with services on this cluster.",
    )
    ...
    force_default_cps = Field(
        False,
        description="When true, changes to the `scaling_groups` will trigger an update of all services on the cluster, forcing them to use the new default capacity provider(s). This setting has no effect if `auto_scaling_enabled == false`.",
        notes=[
    ...
    scaling_groups: List[ScalingGroupModel] = Field(
        [],
        description="One or more `scaling_group` objects defining the EC2 Auto-Scaling Group(s) which will provide container instances for the cluster.",
    )
